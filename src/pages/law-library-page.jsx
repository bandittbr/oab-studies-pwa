    ...
    <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      <section className="glass-panel rounded-[2rem] p-5">
        ...
        <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
          Estrutura pronta para codigos, capitulos e artigos com abertura direta a partir das questoes.
        </p>
        ...
        <div className="mt-5 space-y-3">
          {articles.map((article) => (
            <Link
              ...
              className={"block rounded-[1.4rem] p-4 transition " + (selectedArticle?.id === article.id ? "bg-[var(--text)] text-[var(--bg)]" : "soft-panel")}
            >
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedArticle.keywords.map((keyword) => (
                    <span key={keyword} className="rounded-full border border-[var(--panel-border)] px-3 py-2 text-sm text-[var(--text)]">
                      {keyword}
                    </span>
                  ))}
                </div>
